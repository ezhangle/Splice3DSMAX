...
bool maxCompatibleNormalMapping <
	string UIName = "Use 3ds Max Compatible Normal Mapping";
> = true;
...
float4 std_PS(vertexOutput IN) : COLOR {
	...
	if(g_NormalEnable)
	{
	    ...
	    if (maxCompatibleNormalMapping)
	    {
	    	bump *= 2.0;
	    	Nn = normalize(bump.z*Nn + bump.x*Bn + bump.y*Tn);
	    }
	    else
	    {
